...
{
    textures[key] = std::tuple<std::string, SDL_BlendMode, std::shared_ptr<SDL_Texture>>(std::string(fileName), SDL_BLENDMODE_INVALID, std::shared_ptr<SDL_Texture>(IMG_LoadTexture(renderer.get(), fileName), SDL_DestroyTexture));
    //std::make_pair(std::string(fileName), std::shared_ptr<SDL_Texture>(IMG_LoadTexture(renderer.get(), fileName), SDL_DestroyTexture));
    if (!std::get<2>(textures[key]).get())
    {
    ...
    if (blending_mode != SDL_BLENDMODE_INVALID)
        SDL_SetTextureBlendMode(std::get<2>(textures[key]).get(), static_cast<SDL_BlendMode>(blending_mode & 0xF));

}

std::shared_ptr<SDL_Texture> TextureManager::getTexture(const char* key)
{
    return std::get<2>(textures[key]);
}
